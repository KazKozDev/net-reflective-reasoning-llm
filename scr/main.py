...
class WebSearchManager:
    ...
    def parse_webpage(self, url: str) -> Optional[Dict]:
        ...
        try:
            ...
            if content:
                # Используем только общий парсер
...
class EnhancedLLM:
    ...
    async def evaluate_knowledge(self, query: str) -> Tuple[bool, float, str]:
        """
        Evaluates whether the model has sufficient knowledge to answer the query.
        Returns: (needs_search, confidence, explanation)
        """
        evaluation_prompt = f"""
    ...
    async def process_query(self, query: str) -> Tuple[str, ReasoningChain]:
        ...
        # Initial evaluation of knowledge
        ...
        if not needs_search and confidence > 0.7:
            # If confident, use existing knowledge but still apply detailed reasoning
            ...
            print("\n✨ Refining understanding...", end=' ', flush=True)
            refinement = await self._generate(
                f"Based on our analysis and critique:\n\n"
                f"Analysis: {analysis}\n"
                f"Critique: {critique}\n\n"
                f"Let's refine our understanding. Focus on:\n"
                f"1. Most reliable facts\n"
                f"2. Clear contextual information\n"
                f"3. Specific details we're confident about",
                temp_override=0.4
            )
            reasoning.add_thought(ThoughtProcess.REFINEMENT, refinement, 0.9)
            
        else:
            # Web search process with integrated reasoning
            ...
            for iteration in range(1, self.web_search.max_iterations + 1):
                print(f"\n📊 Search Iteration {iteration}:", end=' ', flush=True)
                
                search_query = await self.web_search.generate_search_query(
                    self, query, user_intent, iteration
                )
                ...
                if not search_results:
                    reasoning.add_thought(
                        ...
                        f"Search iteration {iteration} returned no results. Query: {search_query}",
                        0.5
                    )
                    continue
                ...
                if not iteration_content:
                    reasoning.add_thought(
                        ThoughtProcess.ANALYSIS,
                        f"Could not parse content from search results in iteration {iteration}",
                        0.5
                    )
                    continue
                
                # Analyze search results with detailed reasoning
                analysis = await self._generate(
                    f"Analyze these search results for: {query}\n\n" +
                    "\n".join([
                        f"Source {i+1} ({url}):\nTitle: {content['title']}\n{content['content'][:1000]}"
                        for i, (url, content) in enumerate(iteration_content.items())
                    ]) +
                ...
                results_by_iteration[iteration] = analysis
                reasoning.add_thought(
                    ThoughtProcess.ANALYSIS if iteration == 1 else ThoughtProcess.REFINEMENT,
                    f"Search Iteration {iteration} Analysis:\n{analysis}",
                    0.8
                )
                
            # Critical evaluation of search results
            print("\n⚖️ Evaluating search findings...", end=' ', flush=True)
            critique = await self._generate(
                f"Review all search results and analyses:\n" +
                "\n".join([f"Iteration {i}: {analysis}" for i, analysis in results_by_iteration.items()]) +
                "\n\nCritically evaluate:\n"
        ...
        # Generate final answer combining all reasoning
        ...
        final_prompt = (
            ...
            f"4. Cites sources where relevant\n"
